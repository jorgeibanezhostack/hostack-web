export default async function handler(req, res) {
  ...
  const RESEND_KEY = process.env.RESEND_API_KEY || 're_SRrFnW7m_KZsVNWsyo32dQD91n9JWdP7E';
  ...
  try {
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Hostack <hello@hostack.co>',
        to: ['hello@hostack.co'],
        subject: `New subscriber: ${email}`,
        html: `<p>New founding member signup from <strong>${email}</strong></p><p>Source: hostack.co</p>`
      })
    });
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: `<div style="font-family:'DM Sans',sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#0d1f22">
          <h2 style="color:#019179;margin-bottom:16px">Welcome to Hostack 👋</h2>
